  ...
  const fetchComponent = async () => {
    try {
      const response = await axios.get(
        `${siteConfig.BASE_URL}/${siteConfig.GET_ALL_PROPERTY_TAX_COMPONENT_NAME_MASTER}`
      );
      //filtered with suspended status 0 (active)
      const filteredData = response.data.filter(
        (item) => item.suspendedStatus === 0
      );
      setComponentName(filteredData);
      console.log("component Name: ", filteredData);
    } catch (error) {
        ...
        html={
          <div className="container-fluid">
            <h1 className="heading_h1">
              Master Property Tax Component Name List
            </h1>
